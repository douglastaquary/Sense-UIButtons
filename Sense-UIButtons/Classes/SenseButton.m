...
#import "SenseButton.h"

#if __has_include(<UIColor+SenseColors/UIColor+SenseColors.h>)
#import <UIColor+SenseColors/UIColor+SenseColors.h>
#endif
...
const CGFloat SenseButtonAnimationDuration = 0.15;

@implementation SenseButton
...
- (void)setEnabled:(BOOL)enabled animated:(BOOL)animated
{
    ...
    [UIView animateIf:animated duration:SenseButtonAnimationDuration :^{
        self.alpha = alpha;
    }];
}
...
@implementation SenseUppercaseButton
...
- (void)animateLayer:(CALayer *)layer fromColor:(CGColorRef)fromColor toColor:(CGColorRef)toColor forKey:(NSString *)key
{
    ...
    fade.duration = SenseButtonAnimationDuration;
    [layer addAnimation:fade forKey:key];
}

@end

@interface SenseMenuButton()
@property(nonatomic, strong, readonly) CALayer *backgroundLayer;
@end

@implementation SenseMenuButton
...
@implementation  SenseClearFlatButton
...
- (void)setup
{
    ...
    [self setBackgroundColor:[UIColor clearColor] forState:UIControlStateHighlighted animated:NO];
    [self setBorderColor:[UIColor whiteColor] forState:UIControlStateHighlighted animated:NO];
...
@implementation SenseRedFlatButton
...
- (void)setup
{
   ...
   // self.titleLabel.font = [UIFont sansSerifFontWithSize:15];
    ...
    [self setBackgroundColor:[UIColor redColor] forState:UIControlStateNormal animated:NO];
    [self setBackgroundColor:[UIColor whiteColor] forState:UIControlStateHighlighted animated:NO];
}
